...
async function followRedirects(
  ...
  for (let i = 0; i < maxHops; i++) {
    ...
    try {
      ...
      throw new Error(`followRedirects fetch 失败: url=${url}, error=${e.message}${causeMsg}`);
    }
    jar.absorb(res);
...
export async function casLogin(opts: {
  username: string;
  password: string;
  serviceUrl: string;
}): Promise<CasLoginResult> {
  const { username, password, serviceUrl } = opts;
  ...
  try {
    pageRes = await fetch(loginPageUrl, {
      redirect: 'manual',
  ...
  for (let i = 0; i < 3 && !html.includes('login-croypto'); i++) {
    ...
    if (!loc) break;
    const r = await fetch(new URL(loc, loginPageUrl).toString(), {
      redirect: 'manual',
  ...
  try {
    postRes = await fetch(`${SSO_BASE}/login`, {
      method: 'POST',
      redirect: 'manual',
      headers: {
        'User-Agent': UA,
  ...
  const final = await followRedirects(callbackUrl, jar);
